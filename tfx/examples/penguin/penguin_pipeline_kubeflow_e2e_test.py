...
#     pipeline_name = 'kubeflow-v2-e2e-test-parameter-{}'.format(
...
#     self._run_pipeline(pipeline=kubeflow_pipeline)
#     self.assertTrue(fileio.exists(self._pipeline_root))
...
class PenguinPipelineKubeflowTest(kubeflow_test_utils.BaseKubeflowTest):
  ...
  def setUp(self):
    ...
    # copied to a test-local location for each invocation, and cleaned up at
    # the end of test.
    penguin_test_data_root = os.path.join(penguin_examples_dir, 'data')
    penguin_test_schema_file = os.path.join(penguin_examples_dir, 'schema',
                                            'user_provided', 'schema.pbtxt')
